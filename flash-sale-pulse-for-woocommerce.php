...
function spw_require_file( $relative_path ) {
	...
	add_action( 'admin_notices', function() use ( $relative_path ) {
		if ( current_user_can( 'manage_options' ) ) {
			echo '<div class="notice notice-error"><p>' . esc_html( sprintf( 'Flash Sale Pulse: فایل ضروری پیدا نشد: %s', $relative_path ) ) . '</p></div>';
		}
	});
	return false;
}
...
function spw_run_plugin() {
	if ( ! class_exists( 'WooCommerce' ) ) {
		add_action( 'admin_notices', function() {
			echo '<div class="notice notice-error"><p>' . esc_html__( 'Flash Sale Pulse for WooCommerce requires WooCommerce to be installed and active.', 'flash-sale-pulse-for-woocommerce' ) . '</p></div>';
		});
		return;
	}
